...
static void export_new_var(t_shell *shell, char *str) {
...
static void export_from_envp(t_shell *shell, char *com) {
...
int mx_ush_export(char **args, t_shell *shell) {
    ...
    for (int i = 1; args[i]; i++) {
        if (mx_get_char_index(args[i], '=') < 0) {
            export_from_envp(shell, args[i]);
        }
        if (mx_get_char_index(args[i], '=') > 0)
            export_new_var(shell, args[i]);
    }
